...
def get_starting_points(D, E, theta_ub, num=6):
    ...
    while True:
        ...
        if (
            np.max(np.abs(eigvals(kappa.reshape(D, D)))) < 0.95
            and np.max(
                np.abs(
                    eigvals(
                        kappa.reshape(D, D)[: len(E), : len(E)].reshape(len(E), len(E))
                    )
                )
            )
            < 0.95
...
def get_Ec_volumes(history, d):
    partition = [
        x[1] for x in history[0]
    ]  # assumes that first dimension is E dimension
