    ...
    for i in range(D):
        for j in range(D):
            convolution[i, j, :] = np.sum(
                [fftconvolve(A[i, k, :], B[k, j, :])[:timerange] for k in range(D)],
                axis=0,
            )
    ...
    for i in range(len(history)):
        if i in ic_dims:
            processed_histories.append(
                interval_censor_dimension(history[i], T, partition_length)
            )
        else:
...
def get_effective_history(history, ic_dims, T, p_dt=1):
    ...
    poi_list = list(
        set(
            list(labeled_points_dict.keys())
            + list(labeled_censors_dict.keys())
            + list(grid_dict.keys())
        )
    )
    poi_list.sort()
...
def return_h_and_H(
    kernel, kernel_integral, kernel_parameters, x_range, E, h_dt, T, gamma=1e-4
):
    flag = False
    ...
    pointwise_Phi = lambda t: return_kernel_matrix_at_t(
        t, kernel_integral, kernel_parameters
    )
...
def return_gradient_of_h_recursive(
    ...
    for index, var in enumerate(["theta", "kappa"]):
        for i in range(D):
            for j in range(D):
                deriv_phi_array[index, i, j, :] = return_derivative_of_phi(
                    x_range, kernel_parameters, i, j, E_c, var
                )
    ...
    while True:
        ...
        for index, var in enumerate(["theta", "kappa"]):
            for i in range(D):
                for j in range(D):
                    B[index, i, j, :] = convolve_matrices(
                        phi_E, B[index, i, j, :], h_dt
                    )
                    A[index, i, j, :] = B[index, i, j, :] + convolve_matrices(
                        A[index, i, j, :], phi_E, h_dt
                    )
                    h_grad[index, i, j, :] += A[index, i, j, :]
        ...
        if not return_at_N:  # don't return just at N
            if (
                (np.max(additionals) - running > 100)
                or (n == N)
                or ((perf_counter() - time_start) > 30)
            ):
...
def return_gradient_of_H(
    ...
    for index, var in enumerate(["theta", "kappa"]):
        for i in range(D):
            for j in range(D):
                deriv_Phi_E = return_derivative_of_Phi(
                    x_range, kernel_parameters, i, j, E_c, var
                )
                H_grad[index, i, j, :] = (
                    deriv_Phi_E
                    + convolve_matrices(h_grad[index, i, j, :], Phi_E, h_dt)
                    + convolve_matrices(h, deriv_Phi_E, h_dt)
                )
    ...
    for index, t_data in enumerate(t_arr):
        ...
        for t_p, roles_p in history_prior_to_t:
            for role_p, dim_p in roles_p:
                if role_p == "T" and dim_p in Ec:  # is a Hawkes event?
                    ...
                    A += return_kernel_matrix_at_t(t - t_p, kernel_integral, params)[
                        :, dim_p
                    ]
        ...
        for role_t, dim_t in t_roles:
            ...
            if role_t == "T":  # is a PP dim
                ...
                if index != 0 and len(E) != 0:
                    delta_H = np.diff(
                        get_approx_to_f(t - time_points_up_to_t, h_dt, H, dim_t, E),
                        axis=1,
                    ).T
                    xi += np.sum(
                        (
                            (
                                -1
                                * (
                                    return_nu_over_time(time_points_prior_to_t)
                                    + a_matrix[:, :index]
                                )
                            )[E].T
                        )
                    ...
                    Xi += np.sum(
                        (
                            (
                                ...
                                * (
                                    gammas[:, None]
                                    + return_nu_integral_over_time(
                                        time_points_prior_to_t
                                    )
                                    + A_matrix[:, :index]
                                )
                            )[E].T
        ...
        if index == len(t_arr) - 1:
            ...
            for dim in pp_dims:
                ...
                if len(E) != 0:
                    delta_H = np.diff(
                        get_approx_to_f(t - time_points_up_to_t, h_dt, H, dim, E),
                        axis=1,
                    ).T
                    Xi += np.sum(
                        (
...
def return_nll_with_gradients(
    ...
    ic_compensators = [[] for i in range(D)]  # only non-empty if IC dim
    ic_compensators_grad = [
        [[[[] for _ in range(D)] for _ in range(D)] for _ in range(D)] for _ in range(2)
    ]
    ...
    for index, t_data in enumerate(t_arr):
                    ...
                    for param_type in range(2):
                        for param_i in range(D):
                            for param_j in range(D):
                                a_grad[
                                    param_type, param_i, param_j
                                ] += return_derivative_of_phi_at_t(
                                    t - t_p, params, param_type, param_i, param_j
                                )[
                                    :, dim_p
                                ]
                                A_grad[
        ...
        for param_type in range(2):
            for param_i in range(D):
                for param_j in range(D):
                    a_grad_matrix[param_type, param_i, param_j, :, index] = a_grad[
                        param_type, param_i, param_j
                    ]
                    A_grad_matrix[param_type, param_i, param_j, :, index] = A_grad[
                        param_type, param_i, param_j
                ...
                for param_type in range(2):
                    for param_i in range(D):
                        for param_j in range(D):
                            ...
                            xi_grad += np.sum(
                                get_approx_to_f(
                                    t,
                                    h_dt,
                                    h_grad[param_type, param_i, param_j],
                                    dim_t,
                                    None,
                                )
                                * gammas
                            ...
                            if index != 0 and len(E) != 0:
                                ...
                                h_grad_times_exo_and_kernel = np.sum(
                                    (
                                        (
                                            -1
                                            * (
                                ...
                                xi_grad += (
                                    h_times_kernel_grad + h_grad_times_exo_and_kernel
                                )
                            ppll_grads[param_type, param_i, param_j] += (
                                (-1 / xi) * xi_grad
                            ) * ll_weights_map[dim_t]
                        ...
                        if param_type == 0:
                            ...
                            if index != 0 and len(E) != 0:
                                xi_grad_exo += get_approx_to_f(
                                    t, h_dt, H, dim_t, param_i
                                )
                            ppll_exo_grads[param_i] += (
                                (-1 / xi) * xi_grad_exo
                            ) * ll_weights_map[dim_t]
                ...
                for param_type in range(2):
                    for param_i in range(D):
                        for param_j in range(D):
                            ...
                            ic_compensators_grad[param_type][param_i][param_j][
                                dim_t
                            ].append(Xi_grad)
                        ...
                        for param_j in range(D):
                            ...
                            if len(E) != 0:
                                h_times_Kernel_grad = np.sum(
                                    ...
                                    * np.diff(
                                        get_approx_to_f(
                                            t - time_points_up_to_t, h_dt, H, dim, E
                                        ),
                                        axis=1,
                                    ).T
                                )
    ...
    for dim in ic_dims:
        ...
        for param_type in range(2):
            for param_i in range(D):
                for param_j in range(D):
                    compensator_grads = ic_compensators_grad[param_type][param_i][
                        param_j
                    ][dim]
                    diffs_grad = np.diff(compensator_grads)
                    icll_grads[param_type, param_i, param_j] += ll_weights_map[
                        dim
                    ] * np.sum(diffs_grad * (1 - c_vals / diffs))

                # exo grads
                if param_type == 0:
                    compensator_exo_grads = ic_compensators_exo_grad[param_i][dim]
                    diffs_exo_grad = np.diff(compensator_exo_grads)
                    icll_exo_grads[param_i] += ll_weights_map[dim] * np.sum(
                        diffs_exo_grad * (1 - c_vals / diffs)
                    )
