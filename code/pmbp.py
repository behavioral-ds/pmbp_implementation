...
class PMBP(object):
    ...
    def initialize(self, data_label, history, E, end_train, end_validation, end_test):
        ...
        gamma_start = [[x[0] for x in self.history_train[i]][0] for i in range(len(self.E))]
        gamma_max = [np.max([x[0] for x in self.history_train[i]][:min([len(self.history_train[i]), 10])]) for i in range(len(self.E))]

        for d in self.E:
            train = [x for x in self.history[d] if x[1][1] <= end_train]
            trainval = [x for x in self.history[d] if x[1][1] <= end_validation]
            val = [x for x in self.history[d] if (x[1][0] >= end_train) and (x[1][1] <= end_validation)]
            test = [x for x in self.history[d] if (x[1][0] >= end_validation) and (x[1][1] <= end_test)]
        ...
        for d in self.Ec:
            ...
            val = list(self.history[d][(self.history[d] >= end_train) & (self.history[d] < end_validation)])
            test = list(self.history[d][(self.history[d] >= end_validation) & (self.history[d] < end_test)])
            ...
            gamma_max.append(np.max([x[0] for x in Ec_train[:min([len(Ec_train), 10])]]))
    ...
    def print_parameters(self):
        ...
        print("### ALPHA ###")
        print(np.array(self.parameters[self.D*self.D: 2*self.D*self.D]).reshape(self.D, self.D))
        print("### NU ###")
        print(np.array(self.parameters[2*self.D*self.D:2*self.D*self.D+self.D]))
        
        
    def tune_hyperparameters(self, hyperparameter_grid):
        # perform hyperparameter tuning over the specified hyperparameter_grid
        
        x0_list = get_starting_points(self.D, self.E, self.theta_ub, self.num_starting_points)
        ...
        r_history_train, r_data_label, r_theta_ub, r_end_train, r_E, r_logfit_label = list(zip(
                *repeat([
                    self.history_train,
                    self.data_label,
                    self.theta_ub,
        ...
        pickle.dump(res_outputs, open(f"output/{self.data_label}_hypertuning.p", "wb"))
        ...
        for i in range(len(hyperparameter_grid)):
            nlls = [x[1]["obj_val"] for x in res_outputs][self.num_starting_points*i : self.num_starting_points*(i+1)]
            min_nll = min(nlls)
            min_nll_index = nlls.index(min(nlls))
        ...
        if len(self.Ec) != 0:
            ...
            r_end_train, r_end_validation, r_history_trainval, r_data_label, r_val_volume, r_E = list(zip(
                    *repeat([
                        self.end_train,
                        self.end_validation,
            ...
            r_n, r_end_train, r_end_validation, r_history_trainval, r_data_label, r_val_volume, r_E = list(zip(
                    *repeat([
                        self.n,
                        self.end_train,
                        self.end_validation,
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            if len(self.Ec) != 0:
                once_collection = executor.map(sample_forward_once,
                                           [item for sublist in [[i]*len(hyperparameter_grid) for i in range(self.n)] for item in sublist], # run index
                                           r_end_train,
                                           r_end_validation,
                                           r_history_trainval, 
                                           ...
                                           r_gammas, 
                                           r_data_label,
                                           list(range(len(hyperparameter_grid))) * self.n, # hyperparameter index
                                           r_val_volume,
                                           r_E
                                          )
        ...
        if len(self.Ec) != 0:
            ...
            for hyp_index in range(len(hyperparameter_grid)):
                if hyperparameter_grid[hyp_index][2] == "start":
                    ...
                    gammas = self.gamma_max
                collection_for_hyp = once_collection[hyp_index::len(hyperparameter_grid)]
    ...
    def fit(self, perform_hyperparameter_tuning=False, grid=None):
        ...
        r_gammas, r_history_trainval, r_data_label, r_theta_ub, r_end_validation, r_E, r_logfit_label = list(zip(
                *repeat([
                    gammas,
                    self.history_trainval,
                    self.data_label,
        ...
        res_outputs = list(res_outputs)
        pickle.dump(res_outputs, open(f"output/{self.data_label}_fit_trainval.p", "wb"))
        ...
        pickle.dump(self, open(f"output/{self.data_label}_fittedmodel.p", "wb"))
...
#         self.parameters = np.array([1,1.1,1.2,0.9,0.8,0.7,0.6,0.5,1.3] + [0.1,0.2,0.1,0.2,0.1,0.2,0.1,0.2,0.1] + [0.1,0.2,0.3])
    ...
    def evaluate(self):
        ...
        r_gammas, r_end_validation, r_end_test, r_history, r_data_label, r_test_volume, r_E, r_folder = list(zip(
                *repeat([
                    gammas,
                    self.end_validation,
                    self.end_test,
        ...
        self.test_rmse = get_rmse_error(np.array([x[0] for x in self.history_test_volumes[0]]), test_mean_prediction)
    ...
    def print_performance_metrics(self):
        ...
        print(f"Total Dimension 1 Test Volume: {self.test_volume}. Predicted Total Volume: {prediction}")
    ...
    def get_train_count_predictions(self):
        ...
        h, H, flag = return_h_and_H(exponential_kernel, exponential_kernel_integral, kp, h_grid, self.E, h_dt, self.end_validation, 1e-6)
        if self.gamma_hyp == "start":
            gammas = self.gamma_start
        else:
            gammas = self.gamma_max
        vols = get_volume_per_unit_bet_a_b(0, self.end_validation, self.history, self.parameters, gammas, h, H, h_grid, self.E)
    ...
    def get_test_count_predictions(self):
        ...
        if len(self.Ec) != 0:
            r_gammas, r_end_validation, r_end_test, r_params, r_E = list(zip(
                    *repeat([
                        gammas,
                        self.end_validation,
                        self.end_test,
            ...
            with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
                sim_outputs = executor.map(get_volume_per_unit_bet_a_b_recompute_h,
                                           r_end_validation,
                                           r_end_test,
            ...
            h, H, flag = return_h_and_H(exponential_kernel, exponential_kernel_integral, kp, h_grid, self.E, h_dt, self.end_test, 1e-6)

            self.test_volume_prediction = get_volume_per_unit_bet_a_b(self.end_validation, self.end_test, self.history_traintest, self.parameters, gammas, h, H, h_grid, self.E)
    ...
    def plot_predictions(self):
        ...
        train_volumes = [[x[0] for x in self.history_trainval_volumes[d]] for d in range(self.D)]
        test_volumes = [[x[0] for x in self.history_test_volumes[d]] for d in range(self.D)]
        if self.gamma_hyp == "start":
            gammas = self.gamma_start
        else:
            gammas = self.gamma_max
        ...
        for i in range(self.D):
            ...
            plt.plot(range(1,self.end_validation+1), train_volumes[i], alpha=0.5, color="black", label="data")
            plt.plot([self.end_validation,self.end_validation+1], [train_volumes[i][-1], test_volumes[i][0]], color="black", alpha=0.5)
            plt.plot(range(self.end_validation+1,self.end_test+1), test_volumes[i], alpha=0.5, color="black")
            
            plt.plot(range(0,self.end_validation+1), [gammas[i]] + list(train_predictions[:,i]), color="blue")
            plt.plot([self.end_validation,self.end_validation+1], [train_predictions[-1,i], test_mean_prediction[0,i]], color="blue")
            if len(self.Ec) != 0:
                plt.fill_between(range(self.end_validation+1,self.end_test+1), (test_mean_prediction[:,i]-test_std_prediction[:,i]), (test_mean_prediction[:,i]+test_std_prediction[:,i]), alpha=0.5)  
            plt.plot(range(self.end_validation+1,self.end_test+1), test_mean_prediction[:,i], color="blue", label="PMBP")
            
            plt.axvspan(self.end_validation+1,self.end_test+1, facecolor='gray', alpha=0.2)
